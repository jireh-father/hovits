...
class CgvMovieParser extends CgvParser
{
    ...
    public function extractContentIdInSearch($search_html, $content)
    {
        if (emptyOr($search_html, $content)) {
            throw new CrawlerException('파라미터 에러', array($search_html, $content));
        }
    ...
    /**
     * @param QpWrapper $main_movie_element
     * @return array
     * @throws ParserException
     */
    public function extractRateInMain($main_movie_element)
    {
        if (empty($main_movie_element) || !$main_movie_element->exists()) {
            throw new ParserException('메인 영화박스 없음');
        }

        $score_elem = $main_movie_element->find('.score');
        $this->_exists(compact('score_elem'));

        $point_em_elem = $score_elem->find('.egg-gage .percent');
        $this->_exists(compact('point_em_elem'));

//        $point_em_elem->find('span')->remove();
        $point = (int)$point_em_elem->text();

        $view_em_elem = $score_elem->find('.count .view em');
        $this->_exists(compact('view_em_elem'));
        $count = $view_em_elem->text();
        $count = (int)String::extractNumbers($count);

        return compact('point', 'count');
    }
}